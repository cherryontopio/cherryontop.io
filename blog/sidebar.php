...
foreach ($iterator as $fileInfo) {
    ...
    // Extract the post title (first <h2> tag content)
    if (preg_match('/<h2[^>]*>(.*?)<\/h2>/is', $fileContent, $matches)) {
        $title = strip_tags($matches[1]);
    } else {
        $title = ucwords(str_replace(['-', '_', '.php'], [' ', ' ', ''], $fileInfo->getFilename()));
    }

    // Try to extract a fixed publish date (static value assigned to $publishDate)
    if (preg_match('/\$publishDate\s*=\s*[\'"]([^\'"]+)[\'"]\s*;/', $fileContent, $dateMatch)) {
        $date = $dateMatch[1]; // Static publish date
    } else {
        // If no fixed date is found, use file's modified time as fallback
